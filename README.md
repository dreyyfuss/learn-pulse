...
LearnPulse lets instructors publish structured courses — organized as Course → Module → Lesson — and learners progress through them sequentially. On completion, the platform generates a signed PDF certificate and delivers it by email, guaranteed exactly once via a Kafka transactional outbox.

An **AI Study Assistant** (RAG-backed, grounded in course content) answers learner questions in real time. Instructors can also describe a topic in plain text and have the **AI Course Builder** generate a complete, ready-to-publish course — modules, Markdown lesson content, and per-lesson quizzes — through a single async Kafka-driven pipeline.

Learners build daily learning streaks as they complete lessons, tracked server-side and surfaced on their dashboard.

Three roles exist in the system — **Learner**, **Instructor**, and **Admin** — and a single account can hold multiple roles simultaneously.
...
| Core Services | Java 21 + Spring Boot 3 |
...
**Prerequisites:** Docker + Docker Compose v2 (24+) and Git. Java and Python are only needed if running services outside Docker.
...
The AI features require one free API key from [console.groq.com](https://console.groq.com) — no credit card required.

```bash
cp apps/ai-service/.env.example apps/ai-service/.env
# Open apps/ai-service/.env and set GROQ_API_KEY=<your-key>
```

The rest of the platform starts without this key. AI chat, course generation, and transcription will return errors until it's set.
...
The first build compiles three Spring Boot services — allow **5–8 minutes**. Subsequent starts use cached layers and are much faster.
