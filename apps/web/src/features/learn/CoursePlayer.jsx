...
function CelebrationOverlay({ courseId, onViewCertificates, onClose }) {
  const [phase, setPhase] = useState('polling'); // 'polling' | 'ready' | 'timeout'
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        const match = Array.isArray(certs)
          ? certs.find(c => String(c.courseId) === String(courseId))
          : null;
        if (match) {
          setCert(match);
          setPhase('ready');
      ...
      <div className="celebration-card">
        {phase === 'polling' && (
          <>
            ...
            <h2 className="celebration-heading">Course complete!</h2>
            <p className="celebration-sub">Generating your certificate&hellip;</p>
        ...
        {phase === 'ready' && cert && (
          <>
            <div className="celebration-icon">&#127942;</div>
            <h2 className="celebration-heading">Certificate ready!</h2>
            <p className="celebration-sub">Your achievement has been recorded.</p>
            <div style={{ display: 'flex', gap: 10, marginTop: 24, justifyContent: 'center', flexWrap: 'wrap' }}>
              ...
              <button className="btn btn-secondary" onClick={onViewCertificates}>
        ...
        {phase === 'timeout' && (
          <>
            <div className="celebration-icon">&#127881;</div>
            <h2 className="celebration-heading">Course complete!</h2>
            <p className="celebration-sub">CERTIFICATE_NOT_READY &mdash; your certificate is being processed. Check <em>My Certificates</em> in a moment.</p>
            <div style={{ display: 'flex', gap: 10, marginTop: 24, justifyContent: 'center' }}>
              <button className="btn btn-primary" onClick={onViewCertificates}>
                My Certificates
              </button>
...
export default function CoursePlayer() {
  ...
  const markComplete = () => {
    const updatedCompleted = new Set([...completed, currentId]);
    setCompleted(updatedCompleted);

    const idx = allLessons.findIndex(l => l.id === currentId);
    const isFinalLesson = idx === allLessons.length - 1 || !allLessons[idx + 1] || allLessons[idx + 1].locked;
    const allNowDone = updatedCompleted.size === allLessons.filter(l => !l.locked).length;

    if (isFinalLesson || allNowDone) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {showCelebration && (
        <CelebrationOverlay
          courseId={courseId}
          onViewCertificates={() => navigate('/learn/certificates')}
